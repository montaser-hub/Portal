...
//                     <Text as="h3" MyClass="font-medium" content= "Upcoming Shift" />
//                 </div>
//                 {isStartingSoon && (
...
//                         <div>
//                             <Text as="p" MyClass="text-gray-500 text-sm" content= "Date" />
...
//                           <Text as="p" MyClass="font-medium text-gray-800" content={`${shift.startTime} - ${shift.endTime}`} />
//                         </div>
//                     </div>
//                     <div className={`flex items-center gap-4 p-4 rounded-lg border
//                         ${isStartingSoon ? 'bg-red-50 border-red-200' : 'bg-[${COLORS.primaryLight}] border-gray-200'}`}>
...
//                           <Text as="p" MyClass={`font-semibold ${isStartingSoon ? 'text-red-600' : `text-[${COLORS.primary}]`}`}
            ...
            {shift ? (
                <div className="space-y-4">
                    ...
                    <div className={`flex items-center gap-4 p-4 rounded-lg border
                        ...
                        {/* استخدام Deep Teal الثابت للأيقونة */}
                        <AlertCircle className={`h-5 w-5 ${isStartingSoon ? 'text-red-600' : 'text-[#0F7B8A]'}`} />
                        <div>
                          <Text as="p" MyClass="text-gray-600 text-sm" content="Time until shift" />
                          {/* استخدام Deep Teal الثابت للنص */}
                          <Text as="p" MyClass={`font-semibold ${isStartingSoon ? 'text-red-600' : 'text-[#0F7B8A]'}`}
                          content={timeUntil || 'N/A'} />
                        </div>
                    </div>
                </div>
