...
import {
    ...
    subMonths,
    // **********************************************
    // تم تصحيح الخطأ: يجب التأكد من استيراد addDays
    // **********************************************
    addDays,
} from 'date-fns';
import { Card } from '../../common/Card';
    ...
    const renderDays = () => {
        ...
        while (day <= endDate) {
            for (let i = 0; i < 7; i++) {
                ...
                days.push(
                    ...
                    >
                        <span className={`h-8 w-8 flex items-center justify-center rounded-full
                            ...
                            ${isSameDay(cloneDay, new Date()) ? 'border-2 border-gray-400' : ''}
                            ${isShiftDay ? 'text-white' : ''}
                            `}
                            // استخدام خاصية style لتطبيق لون Deep Teal
                            style={isShiftDay ? { backgroundColor: COLORS.primary } : {}}
                        >
                            {format(cloneDay, 'd')}
                        </span>
                    </div>
                ...
                // **********************************************
                // تم التأكد من استخدام addDays بشكل صحيح هنا
                // **********************************************
                day = addDays(day, 1);
            }
            rows.push(<div className="grid grid-cols-7 gap-1" key={rows.length}>{days}</div>);
            days = [];
        }
        return <div className="space-y-1">{rows}</div>;
    };
