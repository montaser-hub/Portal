...
export const createMultiUserSchedule = async (data, user) => {
  ...
  // Concurrent validations WITHOUT modifying validation function
  const validationResults = await Promise.all(
    documentsToInsert.map(async (doc) => {
      try {
        await overlappingSchedulesValidation(doc); // throws inside
        return null; // no error
      } catch (err) {
        // هنا بنرجّعه كـ object بدل ما نخليه يوقف Promise.all
        return {
          userId: doc.userId,
          date: doc.date,
          shiftId: doc.shiftId,
